...
1. You should see a message that will be similar to this - `Serving HTTP on 0.0.0.0 port 8000 ...`
...
1. Save your file and then refresh the page being served from `http://localhost:8000/index.html`. You should see ten locations marked on your map!

    ![Served page with map](images/multiple-locations.png)

## Adding more data

1. Now that we have access to all the JSON data, we can also plot some of the other data at each data point. Locate the line `icon: emoji`. Add a comma at the end of the line so that we can add another parameter on the line below.

1. Add a line below to plot the description of the penalty on the data marker:
